...
async function openAudit(id){
 ...
 await Promise.all([loadSummary(),loadZones(),loadTeams(),loadRecon()]); $("unifiedImportPanel").hidden=false; if(window.unifiedImporter) await window.unifiedImporter.history();
}
...
window.unifiedImporter=installUnifiedImporter({
  ...
  reloadAfterImport:async()=>{if(currentAudit) await Promise.all([loadSummary(),loadRecon()]);},
  zoneProvider:()=>[],
  teamProvider:()=>[]
});
