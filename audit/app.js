...
function crClampScore(v){return Math.max(0,Math.min(100,Math.round(Number(v)||0)));}
function crScoreLabel(score){
...
function crFindingKey(r){
 const f=String(r?.finding||r?.status||"").trim().toLowerCase().replace(/\s+/g,"_");
 if(f==="matched"||f==="match")return "matched";
 if(f.includes("unlisted"))return "unlisted";
 if(f.includes("missing"))return "missing";
 if(f.includes("short"))return "short";
 if(f.includes("excess"))return "excess";
 const sy=crN(r?.system_qty), ph=crN(r?.physical_qty);
 if(ph<sy)return "short";
 if(ph>sy)return sy===0?"unlisted":"excess";
 return "matched";
}
function crStatusLabel(r){
 const k=crFindingKey(r);
 return ({matched:"Matched",short:"Short",excess:"Excess",missing:"Missing",unlisted:"Unlisted Excess"})[k]||String(r?.finding||"Finding");
}
function crConditionKey(v){
 const s=String(v||"").trim().toLowerCase().replace(/[\s-]+/g,"_");
 if(s.includes("near")&&s.includes("exp"))return "near_expiry";
 if(s.includes("expired"))return "expired";
 if(s.includes("damag"))return "damaged";
 return s||"saleable";
}
function crBuildReportData(){
 const recon=reconRows||[];
 const rc={matched:0,excess:0,short:0,missing:0,unlisted:0,recount:0};
 recon.forEach(r=>{const k=crFindingKey(r);if(k in rc)rc[k]++; if(String(r.count_status||r.status||"").toLowerCase()==="recount")rc.recount++;});
 const physicalConditions={damaged:0,expired:0,near_expiry:0,saleable:0};
 recon.forEach(r=>{const k=crConditionKey(r.condition);if(k in physicalConditions)physicalConditions[k]+=Math.abs(crN(r.physical_qty));else physicalConditions.saleable+=Math.abs(crN(r.physical_qty));});
 ...
 const systemExpiry={expired_lines:expiredStock.length,near_lines:nearStock.length};
 const shortageLoss=recon.filter(r=>crFindingKey(r)==="short"||crFindingKey(r)==="missing").reduce((s,r)=>s+Math.abs(crN(r.variance_value)),0);
 const excessValue=recon.filter(r=>crFindingKey(r)==="excess"||crFindingKey(r)==="unlisted").reduce((s,r)=>s+Math.abs(crN(r.variance_value)),0);
 const damagedValue=recon.filter(r=>crConditionKey(r.condition)==="damaged").reduce((s,r)=>s+Math.abs(crN(r.physical_qty)*crN(r.purchase_rate)),0);
 const expiredValue=expiredStock.reduce((s,r)=>s+Math.abs(crN(r.system_qty)*crN(r.purchase_rate)),0);
 const nearExpiryValue=nearStock.reduce((s,r)=>s+Math.abs(crN(r.system_qty)*crN(r.purchase_rate)),0);
 const systemStockValue=(currentStockRows||[]).reduce((s,r)=>s+Math.abs(crN(r.system_qty)*crN(r.purchase_rate)),0);
 const netInventoryVariance=recon.reduce((s,r)=>s+crN(r.variance_value),0);
 const unvaluedLines=recon.filter(r=>crFindingKey(r)!=="matched" && !(Number(r.purchase_rate)>=0)).length;
 return {recon,reconCounts:rc,physicalConditions,systemExpiry,expiredStock,nearStock,financials:{shortageLoss,excessValue,damagedValue,expiredValue,nearExpiryValue,systemStockValue,netInventoryVariance,unvaluedLines},zones:customerZones||[]};
}
function crBuildAuditHealthScore(data){
 const recon=data.recon||[],rc=data.reconCounts||{},se=data.systemExpiry||{},fin=data.financials||{};
 const total=Math.max(1,recon.length), matched=Number(rc.matched||0), recount=Number(rc.recount||0);
 const accuracy=crClampScore((matched/total)*100);
 const completion=crClampScore(100-((recount/total)*100));
 const damagedLines=recon.filter(r=>crConditionKey(r.condition)==="damaged").length;
 const expiryRiskLines=Number(se.expired_lines||0)+Number(se.near_lines||0);
 const damageRisk=crClampScore(100-((damagedLines/total)*100*4));
 const expiryRisk=crClampScore(100-((expiryRiskLines/total)*100*3));
 const stockValue=Math.max(1,Number(fin.systemStockValue||0));
 const exposure=Math.abs(Number(fin.shortageLoss||0))+Math.abs(Number(fin.excessValue||0))+Math.abs(Number(fin.damagedValue||0))+Math.abs(Number(fin.expiredValue||0))+Math.abs(Number(fin.nearExpiryValue||0));
 const financial=crClampScore(100-Math.min(100,(exposure/stockValue)*100));
 const overall=crClampScore(accuracy*.35+completion*.20+expiryRisk*.15+damageRisk*.10+financial*.20);
 return {overall,accuracy,completion,expiryRisk,damageRisk,financial};
}
function crZoneCompletionRows(){
 return (customerZones||[]).map(z=>{
...
async function generateCustomerReportPdf(){
 ...
 const footer=()=>{const n=doc.getNumberOfPages();for(let i=1;i<=n;i++){doc.setPage(i);doc.setFontSize(7);doc.setTextColor(...grey);doc.text("Confidential - Medvika Healthcare Solutions",margin,290);doc.text(`Page ${i} of ${n}`,pageW-margin,290,{align:"right"});}};
 const newSection=async(title,limit=235)=>{if(y>limit){doc.addPage();await addHeader(false);}doc.setTextColor(...navy);doc.setFontSize(11);doc.setFont("helvetica","bold");doc.text(title,margin,y);y+=3;};
 await addHeader(true);
 const clientName=access?.business_name||access?.client_name||access?.customer_name||currentAudit.project_name||"Customer";
 doc.setFontSize(9);doc.setTextColor(...dark);doc.setFont("helvetica","normal");
 doc.autoTable({startY:y,head:[["Engagement","Details"]],body:[["Client",crSafePdf(clientName)],["Project",currentAudit.project_code||""],["Audit Date",currentAudit.audit_date||""],["Location",currentAudit.location||""],["Audit Status",String(currentAudit.status||"").replaceAll("_"," ")]],theme:"grid",styles:{fontSize:8,cellPadding:2.2},headStyles:{fillColor:navy,textColor:[255,255,255]},columnStyles:{0:{fontStyle:"bold",cellWidth:35}}});
 y=doc.lastAutoTable.finalY+7;
 await newSection("Executive Summary",245);
 const rc=data.reconCounts,pc=data.physicalConditions,se=data.systemExpiry,f=data.financials;
 doc.autoTable({startY:y,head:[["Metric","Count","Metric","Count"]],body:[
  ["Matched",rc.matched,"Excess",rc.excess],["Short",rc.short,"Missing",rc.missing],["Unlisted Excess",rc.unlisted,"Recount",rc.recount],
  ["Damaged Qty",Number(pc.damaged||0).toFixed(3),"Expired Physical Qty",Number(pc.expired||0).toFixed(3)],
  ["Near Expiry Physical Qty",Number(pc.near_expiry||0).toFixed(3),"Saleable Qty",Number(pc.saleable||0).toFixed(3)],
  ["System Expired Lines",se.expired_lines,"System Near Expiry Lines",se.near_lines],
  ["Shortage Loss Ex-GST",crPdfMoney(f.shortageLoss),"Excess Value Ex-GST",crPdfMoney(f.excessValue)],
  ["Damaged Value Ex-GST",crPdfMoney(f.damagedValue),"Expired Value Ex-GST",crPdfMoney(f.expiredValue)],
  ["Net Inventory Variance",crPdfMoney(f.netInventoryVariance),"Near Expiry Exposure",crPdfMoney(f.nearExpiryValue)],
  ["Unvalued Exception Lines",f.unvaluedLines||0,"Valuation Note",(f.unvaluedLines||0)>0?"Some exceptions have no purchase rate":"All material exceptions valued"]
 ],theme:"grid",styles:{fontSize:8,cellPadding:2},headStyles:{fillColor:green,textColor:[255,255,255]}});y=doc.lastAutoTable.finalY+7;
 const score=crBuildAuditHealthScore(data),scoreLbl=crScoreLabel(score.overall)[0];
 await newSection("Executive Audit Intelligence",245);
 doc.autoTable({startY:y,head:[["Audit Health","Accuracy","Completion","Expiry","Damage","Financial"]],body:[[
  `${score.overall}/100 - ${scoreLbl}`,`${score.accuracy}/100`,`${score.completion}/100`,`${score.expiryRisk}/100`,`${score.damageRisk}/100`,`${score.financial}/100`
 ]],theme:"grid",styles:{fontSize:8,cellPadding:2},headStyles:{fillColor:green,textColor:[255,255,255]}});y=doc.lastAutoTable.finalY+7;
 await newSection("Financial Risk & Zone Completion",235);
 const zc=crZoneCompletionRows();
 doc.autoTable({startY:y,head:[["Risk","Value","Zone","Completion"]],body:[
  ["Shortage",crPdfMoney(Math.abs(f.shortageLoss||0)),zc[0]?`${zc[0].zone_code||""} - ${zc[0].zone_name||""}`:"-",zc[0]?`${Number(zc[0].percent||0).toFixed(0)}%`:"-"],
  ["Excess",crPdfMoney(Math.abs(f.excessValue||0)),zc[1]?`${zc[1].zone_code||""} - ${zc[1].zone_name||""}`:"-",zc[1]?`${Number(zc[1].percent||0).toFixed(0)}%`:"-"],
  ["Expired",crPdfMoney(Math.abs(f.expiredValue||0)),zc[2]?`${zc[2].zone_code||""} - ${zc[2].zone_name||""}`:"-",zc[2]?`${Number(zc[2].percent||0).toFixed(0)}%`:"-"],
  ["Damaged",crPdfMoney(Math.abs(f.damagedValue||0)),zc[3]?`${zc[3].zone_code||""} - ${zc[3].zone_name||""}`:"-",zc[3]?`${Number(zc[3].percent||0).toFixed(0)}%`:"-"],
  ["Near Expiry",crPdfMoney(Math.abs(f.nearExpiryValue||0)),zc[4]?`${zc[4].zone_code||""} - ${zc[4].zone_name||""}`:"-",zc[4]?`${Number(zc[4].percent||0).toFixed(0)}%`:"-"]
 ],theme:"grid",styles:{fontSize:8,cellPadding:2},headStyles:{fillColor:navy,textColor:[255,255,255]}});y=doc.lastAutoTable.finalY+7;
 await newSection("Material Reconciliation Exceptions",245);
 const exceptions=data.recon.filter(r=>crFindingKey(r)!=="matched");
 doc.autoTable({startY:y,head:[["Status","Item","Batch","System","Physical","Variance"]],body:exceptions.map(r=>[
  crStatusLabel(r),crSafePdf(r.item_name||""),crSafePdf(r.batch_no||"-"),crSafePdf(pharmacyQtyDisplay(r.system_qty,r.pack_size,r.pack_uom||"Pack")),crSafePdf(pharmacyQtyDisplay(r.physical_qty,r.pack_size,r.pack_uom||"Pack")),crSafePdf(varianceUnitDisplay(r.physical_qty,r.system_qty,r.pack_size))
 ]),theme:"striped",styles:{fontSize:7,cellPadding:1.6,overflow:"linebreak"},headStyles:{fillColor:navy,textColor:[255,255,255]},columnStyles:{1:{cellWidth:55},2:{cellWidth:27}},margin:{left:margin,right:margin,top:31,bottom:15}});y=doc.lastAutoTable.finalY+7;
 await newSection("Expiry & Condition Findings",235);
 const findings=data.recon.filter(r=>["damaged","expired","near_expiry"].includes(crConditionKey(r.condition)));
 doc.autoTable({startY:y,head:[["Item","Batch","Expiry","Qty","Condition"]],body:findings.map(r=>[crSafePdf(r.item_name||""),crSafePdf(r.batch_no||"-"),r.expiry_date||"-",crSafePdf(pharmacyQtyDisplay(r.physical_qty,r.pack_size,r.pack_uom||"Pack")),crSafePdf(crConditionKey(r.condition).replaceAll("_"," "))]),theme:"grid",styles:{fontSize:7,cellPadding:1.6},headStyles:{fillColor:green,textColor:[255,255,255]},margin:{left:margin,right:margin,top:31,bottom:15}});y=doc.lastAutoTable.finalY+7;
 await newSection("Zone Completion",225);
 doc.autoTable({startY:y,head:[["Zone","Category","Status","Supervisor"]],body:(customerZones||[]).map(z=>[crSafePdf(`${z.zone_code||""} - ${z.zone_name||""}`),crSafePdf(z.category||"-"),crSafePdf(z.status||"-"),crSafePdf(z.assigned_supervisor||"-")]),theme:"grid",styles:{fontSize:7.5,cellPadding:1.8},headStyles:{fillColor:navy,textColor:[255,255,255]}});y=doc.lastAutoTable.finalY+7;
 await newSection("Management Observations",215);
 doc.setTextColor(...dark);doc.setFontSize(8);doc.setFont("helvetica","normal");
 const exceptionsCount=rc.short+rc.excess+rc.missing+rc.unlisted;
 const observations=`${exceptionsCount} material reconciliation exception${exceptionsCount===1?"":"s"} identified. Financial risk exposure is ${crPdfMoney(Math.abs(f.shortageLoss)+Math.abs(f.excessValue)+Math.abs(f.damagedValue)+Math.abs(f.expiredValue)+Math.abs(f.nearExpiryValue))}. Smart Quantity normalization is applied to pack/loose quantities. Final financial valuation uses purchase rate excluding GST where available.`;
 const lines=doc.splitTextToSize(crSafePdf(observations),180);doc.text(lines,margin,y);y+=Math.max(12,lines.length*4)+5;
 doc.setDrawColor(215,225,231);doc.line(margin,y,pageW-margin,y);y+=8;doc.setFontSize(8);doc.setTextColor(...dark);
 doc.text("Medvika Audit Lead: ______________________________",margin,y);doc.text("Client Representative: __________________________",110,y);y+=14;
 doc.text("Signature: __________________________",margin,y);doc.text("Signature: __________________________",110,y);y+=14;
 doc.setFontSize(7.5);doc.setTextColor(...grey);doc.text("Valuation basis: Purchase Rate excluding GST. GST % is stored separately where provided.",margin,y);y+=4;doc.text("Detailed item-and-batch reconciliation is available in the accompanying CSV export.",margin,y);
 footer();doc.save(`Medvika_Final_Audit_Report_${currentAudit.project_code||"Audit"}.pdf`);
}
